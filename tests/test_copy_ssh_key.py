...
def ops_cm_ipconfig(dut01, control):
    ...
    assert retstruct.returnCode() == 0, 'Configuring IP address on '\
                                        'the switch port failed!'
...
def check_copy_public_key_playbook(control):
    ...
    '''Run playbook to copy ssh key to authorized keys in switch
     ...
     when the docker instance is initiated but can not be done through
     test script. '''
    info("\n\n### Copying the ssh key from control "
         "machine to switch using a playbook ###\n\n")
    ...
    out = control.cmd("ANSIBLE_SCP_IF_SSH=y ansible ops -m ping")
    print out
    assert '"ping": "pong"' in out, "Test to check the SSH "\
...
class TestAnsibleOpsConfig():
    ...
    def testopsansible(self):
        ...
        ops_cm_ipconfig(dut01obj, controlobj)
        check_copy_public_key_playbook(controlobj)
