...
#
# Links
[force_name=eth0] ops1:eth1
ops1:eth1 -- hs1:eth1
"""


HS1_IP_MASK = '192.168.1.2/24'
OPS1_IP_MASK = '192.168.1.3/24'
PINGCNT = 4


# Function to add an IP address to switch interface mgmt
...
def host_ansbile_defaults(hs1, step):
    ...
    hs1("echo \"[ops]\n192.168.1.3 ansible_ssh_port=22\" > /etc/ansible/hosts")

    # Run the shell script to export ansible variable, ANSIBLE_SCP_IF_SSH=y
    hs1(". env_ansible.sh")
...
def host_ssh_config(hs1, step):
    ...
    # generate ssh key
    hs1("ssh-keygen -t rsa -f id_rsa -N ''")
...
def copy_ssh_key_ansible(hs1, step):
    ...
    step("Running the playbook on the openswitch to copy ssh key\n")
    out = hs1("ansible-playbook /ansible/copy_public_key.yml -u root")
    assert "unreachable=0" in out, \
           "Copying the ssh key from control machine to OPS failed!!"


def ping_ops_ansible(hs1, step):
    # run ansible ping module in a playbook to check the connectivity
    step("Running the ansible playbook to ping the opwnswitch\n")
    out = hs1("ansible-playbook /ansible/ping.yml")
    assert "unreachable=0" in out, "Connecting to the switch failed!!"
